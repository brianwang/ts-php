...
class Mongo_db {
    ...
    public function search($collection, $text = "", $field = array(), $flag = "su") {
        ...
        while ($documents->hasNext()) {
            if (Config::Mongo('mongo_return') == 'object') {
    ...
    public function save($collection = "", $data = array(), $options = array()) {
        ...
        try {
            //$options = array_merge($options, array($this->query_safety => TRUE, 'multiple' => TRUE));
            $this->db->{$collection}->save($this->updates);
            $this->_clear();
            return (TRUE);
        } catch (MongoCursorException $e) {
            show_error("Update of data into MongoDB failed: {$e->getMessage()}", 500);
    ...
    private function connect() {
        ...
        try {
            ...
            if (Config::Mongo('mongo_supress_connect_error')) {
    ...
    private function connection_string() {
        $this->host = trim(Config::Mongo('mongo_host'));
        $this->port = trim(Config::Mongo('mongo_port'));
        $this->user = trim(Config::Mongo('mongo_user'));
        $this->pass = trim(Config::Mongo('mongo_pass'));
        $this->dbname = trim(Config::Mongo('mongo_db'));
        $this->persist = trim(Config::Mongo('mongo_persist'));
        $this->persist_key = trim(Config::Mongo('mongo_persist_key'));
        $this->query_safety = trim(Config::Mongo('mongo_query_safety'));
        $dbhostflag = (bool) Config::Mongo('host_db_flag');
